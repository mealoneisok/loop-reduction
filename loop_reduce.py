# we can reduce any enumerating loops into at most 2 loops by using the following methods
...
def cartesian_product(s_lst, e_lst):
    ...
    for s, e in zip(s_lst, e_lst):  #if any ending index is smaller than the starting index, the loop doesn't run
        if s > e:
            return
